...
def excute(actions, count):
    ...
    for x in actions:
        if flag:
            ...
            driver.teardown()
            tkMessageBox.showerror('message', '已终止运行')
            return
    reporter.set_end_time()
    driver.teardown()
...
run_button = Button(button_frame, width=15, text='运行')
run_button.bind('<Button-1>', run)
run_button.pack(side=LEFT, padx=20)
cancel_button = Button(button_frame, width=15, text='停止')
cancel_button.bind('<Button-1>', stop)
cancel_button.pack(side=LEFT, padx=20)
show_button = Button(button_frame, width=15, text='显示结果')
show_button.bind('<Button-1>', show_result)
show_button.pack(side=LEFT, padx=20)
update_button = Button(
    button_frame, width=15, text='上传数据')
